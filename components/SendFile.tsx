...
const SendFile = () => {
  ...
  const selectedEmailFun = (em: string) => {
    setSelectedEmail(em);
    setSearchTerm('');
  }
    ...
    <>
      <div className="border-gray-200 border-2">
        <input
          ...
          className="w-72 border-gray-200 border-2 p-1 m-2 ml-4 focus:outline-none shadow-md focus:border-gray-300 focus:border-2 rounded-md"
          value={searchTerm}
          onChange={handleSearchChange}
        />
        {searchTerm &&
          <ul className="p-2 w-72 m-2 ml-4 shadow-xl max-h-60 overflow-auto">
            {filteredEmails.map((email: { id: React.Key | null | undefined; email: string }) => (
              <li className="p-1  m-1 cursor-pointer" key={email.id} onClick={() => { selectedEmailFun(email.email) }}>
                {email.email}
                <hr />
              </li>
      ...
      <div className="border-gray-200 border-2 w-full col-span-2">
        <Fileselect sender={user ? { email: user.email ?? '' } : null} email={selectedEmail} />
      </div>
    </>
  );
}
